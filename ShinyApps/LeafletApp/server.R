...
# Render frequency selector
output$frequency.control_occ <- renderUI({
  frequencies <- unique(as.character(dataset_input_occ()$Sampling.Type))
  selectInput("select_time_occ", label = "Sampling Frequency",
              choices=frequencies, selected=1)
})

# Render time selector
output$time.control_occ <- renderUI({
  tmin <- min(dataset_input_occ()$timestamp, na.rm=TRUE)
  tmax <- max(dataset_input_occ()$timestamp, na.rm=TRUE)
  selectInput("time_slider_occ", label = "Select Time", choices = as.character(unique(site_selection_occ()$timestamp)))
})
...
output$map_occ <- renderLeaflet({
  ...
  if (!values.2$starting) {
    ...
    # show boundaries for selected species (and if you click on them, it will tell you which specie)
    if (length(input$species_occ)>0) {
      selected_polygons = terrestrial_mammals[terrestrial_mammals$binomial %in% input$species_occ,]
      pal2 <- colorFactor(heat.colors(8), selected_polygons$binomial)
      tmap <- tmap %>%
        addPolygons(data = selected_polygons, weight = 5, fill=FALSE,
                    color=~pal2(binomial),
...
observe({
  #Modify selection based on nulls
  if (is.null(input$red_occ) & is.null(input$guild_occ)) {
    selected.names <- NULL
  } else if (is.null(input$red_occ)) {
    trows <- as.character(present.species()$guild) %in% input$guild_occ
    selected.species <- present.species_occ()[trows,]
    selected.names <- paste(selected.species$genus, selected.species$species)
  } else if (is.null(input$guild_occ)) {
    trows <- as.character(present.specie_occ()$red_list_status_id) %in%
    ...
    if (is.null(guilds) & is.null(reds)) {
      ...
      selected.species <- present.species_occ()[reds,]
      selected.names <- paste(selected.species$genus, selected.species$species)
    } else if (is.null(reds)) {
      selected.species <- present.species_occ()[guilds,]
      selected.names <- paste(selected.species$genus, selected.species$species)
    } else {
      selected.species <- present.species()[guilds & reds,]
      selected.names <- paste(selected.species$genus, selected.species$species)
...
# Subset dataframe for plotting (no time subset)
# Subset by project, site, frequency, and selected species
plotting_dataset_occ <- reactive({
  if (!is.null(input$species_occ)) {
    subset(site_selection_occ(), (Sampling.Type==input$select_time_occ) & (paste(Genus, Species) %in% input$species_occ))
  } else {
    data.frame()
  }
})
...
# Subset dataframe for mapping (time subset)
# Subset by project, site, frequency, selected species, current time
mapping_dataset_occ <- reactive ({
  if (!is.null(input$species_occ) & (!is.null(input$time_slider_occ))) {
    subset(plotting_dataset_occ(), timestamp==input$time_slider_occ)
  } else {
    #subset(plotting_dataset(), rep(FALSE, times=nrow(plotting_dataset())))
    data.frame()
  }
